...
v0.1.0 pre-release. This project is not yet published to npm.
...
| --- | --- | --- |
| `MCPG001` | High, or medium for placeholder, empty, or short values | Secret-like environment variables such as `TOKEN`, `SECRET`, `PASSWORD`, `PASSWD`, `API_KEY`, `APIKEY`, `PRIVATE_KEY`, `CREDENTIAL`, `ACCESS_KEY`, `AWS_*`, `GCP_*`, `GOOGLE_*`, `AZURE_*`, `OPENAI_*`, `ANTHROPIC_*`, `GITHUB_TOKEN`, and `GITLAB_TOKEN`. Values are never printed. |
